...
class PostgresMemoryStore:
    ...
    def ensure_session(self, session_id: str, user_id: str | None = None) -> None:
        ...
        with psycopg.connect(self.dsn) as conn:
            conn.execute(
                """
                INSERT INTO conversation_sessions(session_id, user_id, created_at, updated_at)
                VALUES (%s, %s, %s, %s)
    ...
    def append_message(
        ...
        *,
        ...
        with psycopg.connect(self.dsn) as conn:
            conn.execute(
                """
                INSERT INTO conversation_messages(message_id, session_id, user_id, role, content, metadata, created_at)
                VALUES (%s, %s, %s, %s, %s, %s, %s)
                ON CONFLICT(message_id) DO NOTHING
                """,
                (message_id, session_id, user_id, role, content, dumps_json(metadata or {}), now),
            )
            conn.execute(
    ...
    def get_summary(self, session_id: str) -> str | None:
        with psycopg.connect(self.dsn) as conn:
            row = conn.execute(
                "SELECT summary FROM short_term_memory WHERE session_id = %s",
                (session_id,),
            ).fetchone()
    ...
    def save_summary(self, session_id: str, summary: str) -> None:
        ...
        with psycopg.connect(self.dsn) as conn:
            conn.execute(
                """
                INSERT INTO short_term_memory(session_id, summary, updated_at)
                VALUES (%s, %s, %s)
    ...
    def upsert_long_term_memory(self, item: dict) -> LongTermMemory:
        ...
        with psycopg.connect(self.dsn) as conn:
            row = conn.execute(
                ...
                ON CONFLICT(user_id, memory_type, memory_key) DO UPDATE SET
                    ...
                    updated_at = EXCLUDED.updated_at
                RETURNING memory_id, user_id, memory_type, memory_key, memory_value,
                          confidence, source_message_id, created_at, updated_at
                """,
                (
                    memory_id,
        ...
        return LongTermMemory(
            memory_id=row[0], user_id=row[1], memory_type=row[2], key=row[3], value=row[4],
            confidence=row[5], source_message_id=row[6], created_at=row[7], updated_at=row[8],
        )
    ...
    def list_long_term_memories(self, user_id: str, limit: int = 20) -> list[LongTermMemory]:
        with psycopg.connect(self.dsn) as conn:
            rows = conn.execute(
                """
                SELECT memory_id, user_id, memory_type, memory_key, memory_value,
                       confidence, source_message_id, created_at, updated_at
