...
def initialize_task_store() -> None:
    ...
    if callable(setup):
        ...
        setup()
        logger.info("analysis.task_store.setup.completed store=%s elapsed_ms=%.1f", type(_task_store).__name__, (time.perf_counter() - started) * 1000)
...
async def _set_redis_status(task_id: str, status: str) -> None:
    try:
        ...
        logger.warning("analysis.redis_status.failed task_id=%s status=%s error_type=%s error=%s", task_id, status, type(exc).__name__, exc)
...
async def _prepare_memory(session_id: str, user_id: str | None, query: str) -> str:
    ...
    short_term = render_short_term_context(build_short_term_context(recent, summary=summary, limit=20))
    long_term = ""
    if user_id:
...
async def run_analysis(query: str, *, session_id: str | None = None, user_id: str | None = None) -> dict:
    ...
    logger.info("analysis.start task_id=%s session_id=%s user_id=%s query_length=%d", task_id, session_id, user_id or "-", len(query))
    await _set_redis_status(task_id, "running")
    await _save(TaskRecord(task_id=task_id, status="running"))
    try:
        memory_query = await _prepare_memory(session_id, user_id, query)
        ...
        if settings.environment != "test":
            ...
            if assistant_content:
                await to_thread(_memory_store.append_message, session_id, "assistant", assistant_content, user_id=user_id, metadata={"task_id": task_id, "status": response["status"]})
        ...
        logger.info("analysis.completed task_id=%s session_id=%s status=%s total_elapsed_ms=%.1f", task_id, session_id, response["status"], (time.perf_counter() - request_started) * 1000)
        return response
...
async def get_task(task_id: str) -> dict:
    ...
    return {"task_id": record.task_id, "status": record.status, "result": record.result, "error": record.error, "updated_at": record.updated_at.isoformat()}
