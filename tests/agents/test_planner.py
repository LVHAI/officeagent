...
from app.agents.planner import PLAN_TOOL, create_execution_planner, create_supervisor, extract_execution_plan
                ...
                "tool_calls": [
                    {
                        ...
                        "args": {"tasks": [{"task_id": "k", "agent": "knowledge-agent", "query": "查制度"}]},
                        "id": "plan-1",
                    }
                ],
            }
                ...
                "tool_calls": [
                    {
                        "name": "submit_execution_plan",
                        "args": {
                            "tasks": [{"task_id": "k", "agent": "knowledge-agent", "query": "查制度", "depends_on": None, "parallel_group": None, "constraints": None}],
                            "rationale": None,
                        },
                    }
                ],
...
def test_extract_execution_plan_from_executed_tool_message():
    payload = {"tasks": [{"task_id": "w", "agent": "web-agent", "query": "查最新市场信息"}], "rationale": "需要当前外部信息"}
    result = {"messages": [{"type": "tool", "name": "submit_execution_plan", "content": json.dumps(payload), "tool_call_id": "call-1"}]}
    plan = extract_execution_plan(result)
...
def test_extract_execution_plan_deduplicates_ai_and_tool_message_with_defaults():
    args = {"tasks": [{"task_id": "k", "agent": "knowledge-agent", "query": "查制度"}]}
    executed = {"tasks": [{"task_id": "k", "agent": "knowledge-agent", "query": "查制度", "depends_on": [], "parallel_group": "default", "constraints": {}}], "rationale": ""}
    result = {"messages": [{"type": "ai", "tool_calls": [{"name": "submit_execution_plan", "args": args}]}, {"type": "tool", "name": "submit_execution_plan", "content": json.dumps(executed), "tool_call_id": "call-1"}]}
    plan = extract_execution_plan(result)
...
def test_extract_execution_plan_deduplicates_identical_tool_calls():
    call = {"name": "submit_execution_plan", "args": {"tasks": [{"task_id": "k", "agent": "knowledge-agent", "query": "查制度"}]}}
...
def test_extract_execution_plan_rejects_different_tool_calls():
    result = {"messages": [{"tool_calls": [{"name": "submit_execution_plan", "args": {"tasks": [{"task_id": "k", "agent": "knowledge-agent", "query": "查制度"}]}}, {"name": "submit_execution_plan", "args": {"tasks": [{"task_id": "w", "agent": "web-agent", "query": "查新闻"}]}}]}]}
    try:
...
def test_supervisor_plan_preserves_llm_selected_agents():
    plan = ExecutionPlan.model_validate(
        {"tasks": [{"task_id": "k", "agent": "knowledge-agent", "query": "根据内部知识回答"}, {"task_id": "t", "agent": "tool-agent", "query": "查询客户数据"}]}
    )
