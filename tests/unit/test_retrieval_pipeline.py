...
def test_retrieval_pipeline_applies_metadata_filter():
    chunks = [
        DocumentChunk(id="c1", content="华东销售数据", metadata={"department": "sales"}),
        DocumentChunk(id="c2", content="华东研发数据", metadata={"department": "engineering"}),
    ]
    pipeline = RetrievalPipeline(chunks)
    results = pipeline.retrieve("华东数据", metadata_filter={"department": "sales"})
